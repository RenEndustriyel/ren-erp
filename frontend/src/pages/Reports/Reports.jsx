...
import { getAllCustomerMovements } from "../../lib/movementStore";
...
export default function Reports() {
  ...
  const refresh = () => {
    ...
    setCustomerMovements(getAllCustomerMovements() || []);
  };
  ...
  const customersReport = useMemo(() => customers.map((customer) => {
    ...
    const linkedInvoices = invoices.filter((invoice) =>
      String(invoice?.customerId || invoice?.supplierId || "") === String(customer?.id)
    );
    const snapshot = getCustomerBalanceSnapshot({
      customer,
      movements: rows,
    ...
    return {
      ...
      code: customer.code || "-",
      debt: snapshot.movementDebt,
      credit: snapshot.movementCredit,
  ...
  const exportCurrent = () => {
    ...
    if (view === "customers") return downloadCsv("ren-cari-raporu.csv", customersReport.map((x) => ({ Kod: x.code, Cari: x.name, HareketBorcu: x.debt.toFixed(2), HareketAlacagi: x.credit.toFixed(2), CariBakiye: x.balance.toFixed(2) })));
          ...
          <>
            ...
            {view === "customers" && (
              <>
                ...
                <div className="report-panel"><ReportTable headers={["Kod", "Cari", "Hareket Borcu", "Hareket Alacağı", "Cari Bakiye"]} rows={customersReport} renderRow={(row) => <><td className="mono">{row.code}</td><td><button type="button" className="table-link" onClick={() => navigate(`/customers/detail?id=${row.id}`)}>{row.name}</button></td><td>{money(row.debt)}</td><td>{money(row.credit)}</td><td className="amount">{money(row.receivable || row.payable)}</td></>} empty="Cari kaydı bulunamadı." /></div>
              </>
            )}
