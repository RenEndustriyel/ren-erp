...
export default function NewSale() {
  ...
  const addProduct = (product) => {
    ...
    setItems((current) => {
      ...
      /*
       * REN ERP satış ekranında Birim Fiyat
       * HER ZAMAN KDV DAHİL müşteri fiyatıdır.
       *
       * Ürün KDV Hariç kayıtlıysa:
       * 100 TL -> 120 TL
       *
       * Ürün KDV Dahil kayıtlıysa:
       * 120 TL -> 120 TL
       */
      ...
      return [
        ...current,
        {
          ...
          // Satırda ve satış kaydında müşteri fiyatı
          // (KDV dahil) tutulur.
          unitPrice:
            customerUnitPrice,
  ...
  const lines = useMemo(
    () =>
      items.map((item) => {
        ...
        // unitPrice artık HER ZAMAN KDV dahil müşteri fiyatıdır.
        const customerUnitPrice =
        ...
        if (manualNet !== null) {
          ...
          // Manuel Satır Toplamı da KDV dahil toplamdır.
          total =
            manualTotal;
  ...
  const createSale = async () => {
    ...
    try {
        ...
        const {
          ...
          .insert({
            ...
            reference_id:
              saleId,
            note:
              `${salePayload.sale_no} satış çıkışı`,
            created_at: now,
          });
      ...
      setSuccess(
        `${salePayload.sale_no} başarıyla oluşturuldu.`
      );
      ...
      /*
       * Bu ekran doğrudan transaction kullanmıyor.
       * Beklenmeyen hata halinde satış kaydı oluştuysa
       * mümkün olduğu kadar tersine çevirmeyi deneriz.
       */
      if (createdSaleItems.length) {
        await supabase
          .from("sale_items")
          .delete()
          .eq(
      ...
      for (
        const previous of previousStocks
      ) {
        await supabase
          .from("products")
          .update({
        ...
        <main className="new-sale-main">
          ...
          <section className="new-sale-card">
            ...
            {selectedCustomer ? (
              ...
              <div className="new-sale-default-customer">
                <div>
                  <span>Varsayılan müşteri</span>
                  <strong>Perakende Müşteri</strong>
                </div>
                <MdPerson />
              </div>
            )}
                  ...
                  {lines.length === 0 ? (
                    ...
                    lines.map((item) => (
                      ...
                      >
                        ...
                        <div className="new-sale-line-net">
                          ₺ {money(item.net)}
                        </div>
          ...
          <section className="new-sale-total-card">
            ...
            <div className="new-sale-total-header">
              <div>
                ...
                <small>Ürün bazlı KDV ve iskonto</small>
              </div>
              <MdPointOfSale />
            </div>
